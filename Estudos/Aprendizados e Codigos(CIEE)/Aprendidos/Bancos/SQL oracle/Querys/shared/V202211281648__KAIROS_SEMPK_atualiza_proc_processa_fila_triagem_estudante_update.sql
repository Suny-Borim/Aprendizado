-- Procedure que consome a fila para ACAO='A'
create or replace procedure proc_processa_fila_triagem_estudante_update
    is
...
begin
    ...
    delete from service_vagas_dev.fila_triagem_estudante where rownum <= 1000 and acao = 'A' returning id_estudante -- na mesma execução de 50 em 50! logando as operações
        bulk collect into l_rows; -- considerando um máximo de 50 linhas processadas por interação

    while l_rows.count > 0 loop
            BEGIN
                service_vagas_dev.proc_atualizar_triagem_estudante_lista_update(l_rows);
            EXCEPTION
                WHEN OTHERS THEN
...
end proc_processa_fila_triagem_estudante_update;