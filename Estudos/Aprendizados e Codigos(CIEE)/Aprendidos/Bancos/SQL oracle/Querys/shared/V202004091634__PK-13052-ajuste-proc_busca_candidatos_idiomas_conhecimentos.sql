create or replace procedure                                     {{user}}.PROC_BUSCA_CANDIDATOS(
    P_STATUS_ESCOLARIDADE {{user}}.IDS_TYP,
...
begin
    ...
    IF (P_CAPACITACAO.LATITUDE1 IS NOT NULL AND P_CAPACITACAO.LONGITUDE1 IS NOT NULL AND P_REFERENCIA_DISTANCIA IN (2, 5, 6)) THEN
        v_cap1_referencia := SDO_GEOMETRY(2001, 8307, SDO_POINT_TYPE(P_CAPACITACAO.LONGITUDE1, P_CAPACITACAO.LATITUDE1, NULL), NULL, NULL);
    END IF;

    IF (P_CAPACITACAO.LATITUDE2 IS NOT NULL AND P_CAPACITACAO.LONGITUDE2 IS NOT NULL AND P_REFERENCIA_DISTANCIA IN (2, 5, 6)) THEN
        v_cap2_referencia := SDO_GEOMETRY(2001, 8307, SDO_POINT_TYPE(P_CAPACITACAO.LONGITUDE2, P_CAPACITACAO.LATITUDE2, NULL), NULL, NULL);
    END IF;

    IF (P_CAPACITACAO.LATITUDE3 IS NOT NULL AND P_CAPACITACAO.LONGITUDE3 IS NOT NULL AND P_REFERENCIA_DISTANCIA IN (2, 5, 6)) THEN
        v_cap3_referencia := SDO_GEOMETRY(2001, 8307, SDO_POINT_TYPE(P_CAPACITACAO.LONGITUDE3, P_CAPACITACAO.LATITUDE3, NULL), NULL, NULL);
    END IF;
    ...
    SELECT te.id_estudante,
               ...
               te.data_alteracao,
               CASE
                   WHEN te.pcds IS NULL OR te.pcds IS EMPTY THEN 1
                   ELSE 0
                   END pcd,
               (SELECT CAST(COLLECT(id_cid_agrupado) AS {{user}}.IDS_TYP) FROM TABLE(pcds)) ids_cid_agrupado,
               (SELECT CAST(COLLECT(agrupamento) AS {{user}}.NAMELIST) FROM TABLE(pcds) p INNER JOIN rep_agrupamento_cid_pcd acd ON acd.codigo_agrupamento = p.id_cid_agrupado) descricao_cid_pcd,
    ...
    if (P_TIPO_PERFIL = 'E') THEN
        ...
        l_query := l_query || CASE P_REFERENCIA_DISTANCIA
                                  WHEN 0 THEN '(SDO_GEOM.SDO_DISTANCE(:P_REFERENCIA, TE.ENDERECO, 0.005, ''unit=km'') < :p_valor_raio and (:p_referencia is not null and :p_valor_raio is not null))'
                                  WHEN 1 THEN '(SDO_GEOM.SDO_DISTANCE(:P_REFERENCIA, TE.ENDERECO_CAMPUS, 0.005, ''unit=km'') < :p_valor_raio and (:p_referencia is not null and :p_valor_raio is not null))'
                                  WHEN 2 THEN '(SDO_GEOM.SDO_DISTANCE(:P_REFERENCIA, TE.ENDERECO, 0.005, ''unit=km'') < :p_valor_raio) AND (SDO_GEOM.SDO_DISTANCE(:P_REFERENCIA, TE.ENDERECO_CAMPUS, 0.005, ''unit=km'') < :p_valor_raio)'
    ...
    ELSE
        ...
        l_query := l_query || CASE
                                  WHEN P_REFERENCIA_DISTANCIA IN (0, 5) THEN '(SDO_GEOM.SDO_DISTANCE(:P_REFERENCIA, TE.ENDERECO, 0.005, ''unit=km'') < :p_valor_raio and (:p_referencia is not null and :p_valor_raio is not null))'
                                  WHEN P_REFERENCIA_DISTANCIA IN (1, 6) THEN '(SDO_GEOM.SDO_DISTANCE(:P_REFERENCIA, TE.ENDERECO_CAMPUS, 0.005, ''unit=km'') < :p_valor_raio and (:p_referencia is not null and :p_valor_raio is not null))'
                                  WHEN P_REFERENCIA_DISTANCIA = 3 THEN '(SDO_GEOM.SDO_DISTANCE(:P_REFERENCIA, TE.ENDERECO, 0.005, ''unit=km'') < :p_valor_raio) AND (SDO_GEOM.SDO_DISTANCE(:P_REFERENCIA, TE.ENDERECO_CAMPUS, 0.005, ''unit=km'') < :p_valor_raio)'
                                  WHEN P_REFERENCIA_DISTANCIA = 4 THEN '(SDO_GEOM.SDO_DISTANCE(:P_REFERENCIA, TE.ENDERECO, 0.005, ''unit=km'') < :p_valor_raio) OR (SDO_GEOM.SDO_DISTANCE(:P_REFERENCIA, TE.ENDERECO_CAMPUS, 0.005, ''unit=km'') < :p_valor_raio)'
                                  ELSE ''
            end;
