CREATE OR REPLACE VIEW V_ICN_LOCAL_CONTRATO AS 
  SELECT DISTINCT ROWNUM AS ID,
        LC.CNPJ,
        LC.RAZAO_SOCIAL,
        END.UF,
        LC.ID AS ID_LOCAL_CONTRATO,
        C.ID AS ID_CONTRATO,
        C.SITUACAO_CONTRATO,
  ...
  INNER JOIN REP_LOCAIS_REPRESENTANTES RCA ON RCA.ID_LOCAL_CONTRATO = LC.ID AND RCA.TIPO = 'REPRESENTANTE ADMINISTRATIVO'
  INNER JOIN REP_REPRESENTANTES RPA ON RPA.ID = RCA.ID_REPRESENTANTE
  INNER JOIN REP_LOCAIS_REPRESENTANTES RCL ON RCL.ID_LOCAL_CONTRATO = LC.ID AND RCL.TIPO = 'REPRESENTANTE LEGAL'
