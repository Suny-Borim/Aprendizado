CREATE or replace VIEW V_DADOS_IE_ICN
AS (
    select rownum                                               as id
         ...
         , t.id_unidade
         , t.id_ie
         , SUBSTR2(t.contato, 0, 50)                            as contato
         , SUBSTR2(t.razao_social, 0, 70)                       as razao_social
         , case when acordo.deletado = 1 then 'A' else 'I' end  as situacao_conv
         , acordo.data_criacao                                  as data_conv
         , t.cnpj_ie
         , t.cep
         , SUBSTR2(t.endereco, 0, 100)                          as endereco
         , SUBSTR2(t.bairro, 0, 50)                             as bairro
         , SUBSTR2(t.cidade, 0, 50)                             as cidade
         , t.nivel
         , t.tel

    from (
             select un.id_unidade_administrativa as id_unidade
                  , cee.id_ie                    as id_ie
                  , p.nome                       as contato
                  ...
                  , BUSCAR_TELEFONE_IE(ie.id)     as tel
