create or replace view V_DADOS_ESTAGIARIOS_ICN
    as
    (
        select rownum                                                  as id
             , SUBSTR2(stag.nome, 0, 50)                               as nome
             , stag.cpf
             , stag.codigo_estudante
             , stag.data_criacao
             , stag.data_alteracao
             , stag.email
             , stag.data_nascimento
             , stag.rg
             , stag.sexo
             , stag.uf_rg
             , stag.orgao_emissor_rg
             , stag.id_unidade
             , 'A'                                                     AS situacao
             , stag.id_agencia
             , stag.id_banco
             , stag.conta_corrente
             , stag.conta_corrente_digito
             , stag.digito_agencia
             , stag.cep
             , SUBSTR2(stag.bairro, 0, 50)                             as bairro
             , SUBSTR2(stag.endereco, 0, 100)                          as endereco
             , stag.numero
             , SUBSTR2(stag.CIDADE, 0, 50)                             as cidade
             , stag.uf

             , case when stag.id_unidade = 101 then 'sp' else 'rj' end as uf_unidade
        from (
                 SELECT con.ID_BANCO
                      , con.ID_AGENCIA
                      , con.CONTA_CORRENTE
                      , con.CONTA_CORRENTE_DIGITO
                      , e.NOME
                      , e.SEXO
                      , e.CPF
                      , e.DATA_NASCIMENTO
                      , e.RG
                      , e.UF_RG
                      , e.ORGAO_EMISSOR_RG
                      , e.CODIGO_ESTUDANTE
                      , e.DATA_CRIACAO
                      , e.DATA_ALTERACAO
                      , en.BAIRRO
                      , en.ENDERECO
                      , en.NUMERO
                      , en.CIDADE
                      , en.CEP
                      , en.UF
                      , a.DIGITO_AGENCIA
                      , email.EMAIL
                      , un.id_unidade_administrativa as id_unidade

                 FROM REP_ESTUDANTES e
                          INNER JOIN CONTRATOS_ESTUDANTES_EMPRESA con ON con.ID_ESTUDANTE = e.ID
                          INNER JOIN REP_ENDERECOS_ESTUDANTES en ON en.ID_ESTUDANTE = e.ID
                          INNER JOIN REP_EMAILS email ON email.ID_ESTUDANTE = e.ID
                          INNER JOIN rep_locais_contrato lc on lc.id = con.id_local_contrato
                          INNER JOIN rep_locais_enderecos enlc on enlc.id_local_contrato = lc.id
                          INNER JOIN rep_unidades_ciee un on un.id = enlc.id_unidade_ciee
                          LEFT JOIN AGENCIAS a ON con.ID_AGENCIA = a.ID_AGENCIA and a.ID_BANCO = con.ID_BANCO

                 WHERE en.PRINCIPAL = 1
                   AND email.principal = 1
                   AND con.tipo_contrato = 'E'
                   AND con.SITUACAO in (0 , 1, 2)
             ) stag)
;
