create or replace FUNCTION create_geohash_2(latitude FLOAT, longitude FLOAT, radius INT, precision INT) RETURN GEOHASHS_TYP
    IS
    x           FLOAT         := 0.0 ;
    y           FLOAT         := 0.0 ;
    ...
    grid_width  grid_width_t  := grid_width_t(5009400.0, 1252300.0, 156500.0, 39100.0, 4900.0, 1200.0, 152.9, 38.2, 4.8,
                                              1.2, 0.149, 0.0370);
    TYPE grid_height_t IS TABLE OF FLOAT;
    grid_height grid_height_t := grid_height_t(4992600.0, 624100.0, 156000.0, 19500.0, 4900.0, 609.4, 152.4, 19.0, 4.8,
                                               0.595, 0.149, 0.0199);
    height      FLOAT;
    width       FLOAT;
    lat_moves   INT;
    ...
    --TYPE geohash_t IS TABLE OF VARCHAR(7);
    geohash     GEOHASHS_TYP  := GEOHASHS_TYP();
