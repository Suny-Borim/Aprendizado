...
BEGIN
    ...
    {{user}}.proc_atualizar_triagem_estudante_remove_inativo(V_IDS_ESTUDANTES_INTERNO);
    ...
    l_query := 'MERGE INTO {{user}}.TRIAGENS_ESTUDANTES D
        ...
        -- #### Info (experiências, idiomas, conhecimentos) ####
            ...
            XmlElement("conhecimentos",(SELECT XMLAGG(XmlElement("conhecimento", RCI.FERRAMENTA)) FROM {{user}}.REP_CONHECIMENTOS_INFORMATICA RCI WHERE RCI.ID_ESTUDANTE = EST.ID)),
            XmlElement("diversos",(SELECT XMLAGG(XmlElement("diverso", RCD.NOME_CURSO)) FROM {{user}}.REP_CONHECIMENTOS_DIVERSOS_STUDENT RCD WHERE RCD.ID_ESTUDANTE = EST.ID)),
            XmlElement("idiomas",(SELECT XMLAGG(XmlElement("idioma", RIN.IDIOMA)) FROM {{user}}.REP_IDIOMAS_NIVEIS RIN WHERE RIN.ESTUDANTE_ID = EST.ID)),
            XmlElement("experiencias",(SELECT XMLAGG(XmlElement("experiencia", REXP.NOME_EMPRESA)) FROM {{user}}.REP_EXPERIENCIAS_PROFISSIONAIS_STUDENT REXP WHERE REXP.ID_ESTUDANTE = EST.ID ))
        ).getClobVal() INFO,
        ...
        case when (EST.SITUACAO = ''ATIVO''
            ...
            AND (Q.DELETADO IS NULL OR Q.DELETADO = ''0'')) then 1
            else 0
        end APTO_TRIAGEM
        FROM
