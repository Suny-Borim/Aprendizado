create or replace procedure                   PROC_BUSCA_CANDIDATOS(
    ...
    P_CONTRATACAO_JOVEM_TALENTO NUMBER,
...
begin
    ...
    SELECT te.id_estudante,
           ...
           distance(:P_REFERENCIA.SDO_POINT.Y, :P_REFERENCIA.SDO_POINT.X, TE.ENDERECO.SDO_POINT.Y, TE.ENDERECO.SDO_POINT.X, 6387.7) AS DISTANCIA,
    ...
    l_query := l_query || '
                from triagens_estudantes te
                     inner join rep_estudantes rep on (rep.id = te.id_estudante and rep.flag_ativo=1)
    ...
    IF (P_TIPO_PERFIL = 'E') THEN
        ...
        IF (P_CONTRATACAO_JOVEM_TALENTO = 1 AND P_AREA_PROFISSIONAL IS NOT NULL) THEN

            l_query := l_query || '
                AND (TE.CONTRATOS_EMPRESA IS EMPTY
                        OR TE.CONTRATOS_EMPRESA IS NULL
                        OR NOT (:P_AREA_PROFISSIONAL member of TE.AREAS_PROFISSIONAIS_JOVEM_TALENTO_CONTRATADO))';
        ELSE

            IF (P_AREA_PROFISSIONAL IS NOT NULL) THEN

                l_query := l_query ||'
                           AND (:P_AREA_PROFISSIONAL IS NOT NULL)
                           ';
            ELSE
                l_query := l_query ||'
                           AND (:P_AREA_PROFISSIONAL IS NULL)
    ...
    l_query := l_query || ' ORDER BY TE.CLASSIFICACAO_OBTIDA ' || v_ordenacao_classificacao || ', TE.PONTUACAO_OBTIDA DESC, DISTANCIA ';
    l_query := l_query || ' OFFSET :P_OFFSET ROWS FETCH NEXT :P_NEXT ROWS ONLY';

    dbms_output.put_line('END L_QUERY');
    dbms_output.put_line(SYSDATE);
    dbms_output.put_line(l_query);
    ...
    IF (P_TIPO_PERFIL = 'E') THEN
        OPEN P_RESULTSET
            FOR l_query
            USING
            V_REFERENCIA,V_REFERENCIA, P_USUARIO
