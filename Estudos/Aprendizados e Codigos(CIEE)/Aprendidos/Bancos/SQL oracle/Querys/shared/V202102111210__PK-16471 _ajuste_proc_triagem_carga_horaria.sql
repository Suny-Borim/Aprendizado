create or replace PROCEDURE proc_atualizar_triagem_estudante_lista_carga_horaria_diaria(V_IDS_ESTUDANTES IN IDS_TYP DEFAULT NULL)
AS
    l_query  CLOB;
...
BEGIN
    ...
    l_query := l_query || '
            ...
            AND esc.PRINCIPAL = 1
            AND (
                (par.id_campus is not null and par.id_campus = rcc.id_campus)
                    or
                ( par.id_campus is null and not exists(select 1 from parametros_ie subPar where subPar.ID_IE = par.ID_IE and subPar.ID_CAMPUS = rcc.id_campus ))
              )
            AND (
                (par.id_curso is not null and par.id_curso = esc.id_curso)
                    or
                (par.id_curso is null and not exists(select 1 from parametros_ie subPar where subPar.ID_IE = par.ID_IE and subPar.id_curso = rcc.id_curso ) )
              )
            AND JSON_VALUE(respostas, ''$.tipo'') IN (''cargaHorariaDiaria'')) S
        ON (D.ID_ESTUDANTE = S.ID_ESTUDANTE)
        WHEN MATCHED THEN
        UPDATE SET
            D.par_carga_horaria_diaria = S.carga_horaria_diaria  -- cargaHorariaDiaria
            ,D.par_nivel_nao_permitido  = S.nivel_nao_permitido   -- cargaHorariaDiaria
            ,D.PAR_AREAS_ATUACAO        = null
            ,D.PAR_SEMESTRE_INICIO      = null
            ,D.PAR_SEMESTRE_FIM         = null
            ,D.PAR_SEMESTRE_MAXIMO      = null
