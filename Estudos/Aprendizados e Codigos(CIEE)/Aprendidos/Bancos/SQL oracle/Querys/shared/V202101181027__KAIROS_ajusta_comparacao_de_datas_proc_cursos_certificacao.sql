       ...
       USING (SELECT id
                ...
                SELECT ni.estudante_id AS ID, COUNT(ni.estudante_id) AS qtd_certificado  
                    ...
                    WHERE ni.estudante_id IN (SELECT COLUMN_VALUE FROM TABLE(p_lista_estudante))
                        ...
                        AND ( CAST(ni.data_alteracao AS DATA) - CAST(sysdate AS DATE)) >= 0 )
                        AND ( CAST(ni.data_alteracao AS DATA) - CAST(sysdate AS DATE)) <= p_periodo )
                        AND ni.deletado = 0
                ...
                SELECT ci.id_estudante AS ID, COUNT(ci.id_estudante) AS qtd_certificado
                    ...
                    WHERE ci.id_estudante IN (SELECT COLUMN_VALUE FROM TABLE(p_lista_estudante))
                        ...
                        AND ( CAST(ci.data_alteracao AS DATA) - CAST(sysdate AS DATE)) >= 0 )
                        AND ( CAST(ci.data_alteracao AS DATA) - CAST(sysdate AS DATE)) <= p_periodo )
                        AND ci.deletado = 0
                ...
                SELECT cd.id_estudante AS ID, COUNT(cd.id_estudante) AS qtd_certificado
                    ...
                    WHERE cd.id_estudante IN (SELECT COLUMN_VALUE FROM TABLE(p_lista_estudante))
                        ...
                        AND ( CAST(cd.data_alteracao AS DATA) - CAST(sysdate AS DATE)) >= 0 )
                        AND ( CAST(cd.data_alteracao AS DATA) - CAST(sysdate AS DATE)) <= p_periodo )
                        AND cd.deletado = 0
