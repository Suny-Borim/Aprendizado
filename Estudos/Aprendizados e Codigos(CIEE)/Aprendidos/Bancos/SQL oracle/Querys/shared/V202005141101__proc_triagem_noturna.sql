...
BEGIN
    ...
    FOR c_vaga IN (SELECT codigo_da_vaga FROM {{user}}."TRIAGENS_VAGAS" WHERE calculo_saldo_vaga (codigo_da_vaga, tipo_vaga) > 0)
        LOOP
            ...
            FROM
                (SELECT /*+ PARALLEL(8) */
                 ...
                 WHERE
                   ...
                   AND (E.CONTRATOS_EMPRESA IS EMPTY OR E.CONTRATOS_EMPRESA IS NULL OR NOT EXISTS (SELECT 1 from TABLE (E.CONTRATOS_EMPRESA) EC_EMP WHERE EC_EMP.ID_EMPRESA = V.ID_EMPRESA AND EC_EMP.SITUACAO <> 0 and EC_EMP.TIPO_CONTRATO = V.TIPO_VAGA))
                   ...
                   AND NOT EXISTS (SELECT 1 FROM {{user}}.VINCULOS_VAGA t WHERE t.codigo_vaga = V.codigo_da_vaga AND t.id_estudante = E.ID_ESTUDANTE AND t.deletado = 0 AND t.SITUACAO_VINCULO <> 3)
                   ...
                   AND (
                     (
                       ...
                       -- Regras específicas para vagas estágio
                       AND (

                         -- [E] Valida escolaridade/curso do estudante está aderente com a vaga
                         (E.ESCOLAS IS NOT NULL AND E.ESCOLAS IS NOT EMPTY)
                         ...
                         AND (
                           V.ESTADO_CIVIL IS NULL
                           OR
                           V.ESTADO_CIVIL IS EMPTY
                           OR
                         ...
                         AND (
                           ...
                           OR
                           (
                             E.ESCOLAS IS NOT NULL
                             AND E.ESCOLAS IS NOT EMPTY
                             AND EXISTS (SELECT 1 FROM TABLE (E.ESCOLAS) WHERE COLUMN_VALUE MEMBER OF (V.ESCOLAS))
                           )
                         )
                         ...
                         AND (
                           V.CONHECIMENTOS IS NULL
                           OR
                           V.CONHECIMENTOS IS EMPTY
                           OR
                           (
                             E.CONHECIMENTOS IS NOT NULL
                             AND E.CONHECIMENTOS IS NOT EMPTY
                             AND EXISTS (
                               SELECT 1 FROM
                               TABLE (E.CONHECIMENTOS) CE
                               INNER JOIN TABLE (V.CONHECIMENTOS) CV ON CE.DESCRICAO = CV.DESCRICAO
                               AND CE.NIVEL >= CV.NIVEL
                             )
                           )
                         )
                         ...
                         AND (
                           ...
                           OR
                           (
                             E.IDIOMAS IS NOT NULL
                             AND E.IDIOMAS IS NOT EMPTY
                             AND EXISTS (
                               SELECT 1 FROM
                               TABLE (E.IDIOMAS) IDE
                               INNER JOIN TABLE (V.IDIOMAS) IDV ON IDE.NOME = IDV.NOME
                               AND IDE.NIVEL >= IDV.NIVEL
                             )
                           )
                         )

                         -- [E] CIRCULO DE DISTANCIA ENTRE O LOCAL DE ESTAGIO E O ENDERECO DO ALUNO
                         AND (
                         ...
                         -- [E] CIRCULO DE DISTANCIA ENTRE O LOCAL DE ESTAGIO E O ENDERECO DO CAMPUS DO ALUNO
                         AND (
                         ...
                         -- TIPO PROGRAMA deve ser ESTAGIO ou ESTAGIO+APRENDIZ
                     ...
                     OR (
                       ...
                       -- Regras específicas para vagas aprendiz
                       OR
                       (
                         -- [A] Validação da escolaridade/situação de escolaridade do candidato
                         ((V.SITUACAO_ESCOLARIDADE = 0) OR (V.SITUACAO_ESCOLARIDADE = 1 and E.STATUS_ESCOLARIDADE = 1) OR (V.SITUACAO_ESCOLARIDADE = 2 and E.STATUS_ESCOLARIDADE = 0))
                         ...
                         -- [A] CIRCULO DE DISTANCIA ENTRE O LOCAL DE ESTAGIO E O ENDEREÇO DO ALUNO
                         AND (
                         ...
                         -- [A] CIRCULO DE DISTÂNCIA ENTRE O LOCAL DE ESTAGIO E O ENDEREÇO DO CAMPUS DO ALUNO
                         AND (
                         ...
                         -- [A] TIPO PROGRAMA deve ser APRENDIZ ou ESTAGIO+APRENDIZ
                         AND E.TIPO_PROGRAMA IN (1, 2)
                       )
                     )
                   )

                   -- Regras comuns de vagas estágio e aprendiz
                   ...
                   AND ROWNUM <= calculo_saldo_vaga (v.codigo_da_vaga, v.tipo_vaga)
