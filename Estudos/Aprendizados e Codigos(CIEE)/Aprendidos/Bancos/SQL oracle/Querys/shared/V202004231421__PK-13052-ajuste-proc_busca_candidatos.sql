create or replace procedure                   {{user}}.PROC_BUSCA_CANDIDATOS(P_STATUS_ESCOLARIDADE {{user}}.IDS_TYP,
                                                                          ...
                                                                          P_PALAVRAS_CHAVE {{user}}.NAMELIST,
...
AS
    ...
    v_geohash_ref             {{user}}.GEOHASHS_TYP;
    v_geohash_cap1            {{user}}.GEOHASHS_TYP;
    v_geohash_cap2            {{user}}.GEOHASHS_TYP;
    v_geohash_cap3            {{user}}.GEOHASHS_TYP;
    v_cursos                  {{user}}.IDS_TYP := p_cursos;
    v_palavras_chave          VARCHAR2(32767);
begin

    v_referencia := SDO_GEOMETRY(2001, 8307, SDO_POINT_TYPE(P_LONGITUDE, P_LATITUDE, NULL), NULL, NULL);
    v_geohash_ref := GEOHASH_NEIGHBORS(geohash_encode(p_latitude, p_longitude, 4));

    IF (P_CAPACITACAO.LATITUDE1 IS NOT NULL AND P_CAPACITACAO.LONGITUDE1 IS NOT NULL AND
        P_REFERENCIA_DISTANCIA IN (2, 5, 6)) THEN
        v_geohash_cap1 := GEOHASH_NEIGHBORS(geohash_encode(P_CAPACITACAO.LATITUDE1, P_CAPACITACAO.LONGITUDE1, 4));
        v_cap1_referencia :=
    ...
    IF (P_CAPACITACAO.LATITUDE2 IS NOT NULL AND P_CAPACITACAO.LONGITUDE2 IS NOT NULL AND
        P_REFERENCIA_DISTANCIA IN (2, 5, 6)) THEN
        v_geohash_cap2 := GEOHASH_NEIGHBORS(geohash_encode(P_CAPACITACAO.LATITUDE2, P_CAPACITACAO.LONGITUDE2, 4));
        v_cap2_referencia :=
    ...
    IF (P_CAPACITACAO.LATITUDE3 IS NOT NULL AND P_CAPACITACAO.LONGITUDE3 IS NOT NULL AND
        P_REFERENCIA_DISTANCIA IN (2, 5, 6)) THEN
        v_geohash_cap3 := GEOHASH_NEIGHBORS(geohash_encode(P_CAPACITACAO.LATITUDE3, P_CAPACITACAO.LONGITUDE3, 4));
        v_cap3_referencia :=
    ...
    select cpp.PONTO_ATE + case
        ...
        END PONTUACAO_MAX,
           case
               ...
               ELSE
                   'ASC'
               end ORDENACAO_CLASSIFICACAO INTO v_max_pontuacao, v_ordenacao_classificacao
    ...
    SELECT te.id_estudante,
           ...
           CASE
               ...
               ELSE 1
               END                                                                                            pcd,
           (SELECT CAST(COLLECT(id_cid_agrupado) AS {{user}}.IDS_TYP)
    ...
    ELSE
        ...
        l_query := l_query || CASE
                                  ...
                                  WHEN P_REFERENCIA_DISTANCIA = 3
                                      THEN '(te.ENDERECO_GEOHASH member of :v_geohash_ref AND SDO_GEOM.SDO_DISTANCE(:P_REFERENCIA, TE.ENDERECO, 0.005, ''unit=km'') < :p_valor_raio) AND (te.ENDERECO_CAMPUS_GEOHASH member of SDO_GEOM.SDO_DISTANCE(:P_REFERENCIA, TE.ENDERECO_CAMPUS, 0.005, ''unit=km'') < :p_valor_raio)'
                                  WHEN P_REFERENCIA_DISTANCIA = 4
                                      THEN '(te.ENDERECO_GEOHASH member of :v_geohash_ref AND SDO_GEOM.SDO_DISTANCE(:P_REFERENCIA, TE.ENDERECO, 0.005, ''unit=km'') < :p_valor_raio) OR (te.ENDERECO_CAMPUS_GEOHASH member of SDO_GEOM.SDO_DISTANCE(:P_REFERENCIA, TE.ENDERECO_CAMPUS, 0.005, ''unit=km'') < :p_valor_raio)'
        ...
        IF (P_REFERENCIA_DISTANCIA IN (2, 5, 6)) THEN
            ...
            IF (v_cap1_referencia IS NOT NULL) THEN
                ...
                END IF;
            ELSE
                l_query := l_query || '(:v_geohash_cap1 IS NULL AND :p_referencia_cap1 is null and :p_valor_raio is not null)';
            END IF;
            ...
            IF (v_cap2_referencia IS NOT NULL) THEN
                ...
                END IF;
            ELSE
                l_query := l_query || ' AND (:v_geohash_cap2 IS NULL AND :p_referencia_cap2 is null and :p_valor_raio is not null)';
            END IF;
            ...
            ELSE
                l_query := l_query || ' AND (:v_geohash_cap3 IS NULL AND :p_referencia_cap3 is null and :p_valor_raio is not null)';
            END IF;
    ...
    IF (P_PALAVRAS_CHAVE IS NOT NULL AND P_PALAVRAS_CHAVE IS NOT EMPTY AND P_ONDE_BUSCAR IS NOT NULL) THEN
        ...
        IF (P_TODAS_PALAVRAS IS NULL OR P_TODAS_PALAVRAS = 0) THEN
            select '(' || listagg(column_value, ' AND ') within group (order by column_value) || ')' into v_palavras_chave from table(p_palavras_chave);
        ELSE
            select '"' ||listagg(column_value, ' ') within group (order by column_value) || '"' into v_palavras_chave from table(p_palavras_chave);
        END IF;
        ...
        l_query := l_query || '
             AND catsearch(te.info, ''<query>
                   ...
                   CASE
                       WHEN P_ONDE_BUSCAR = 0 THEN v_palavras_chave || ' within conhecimentos OR '|| v_palavras_chave || ' within diversos'
