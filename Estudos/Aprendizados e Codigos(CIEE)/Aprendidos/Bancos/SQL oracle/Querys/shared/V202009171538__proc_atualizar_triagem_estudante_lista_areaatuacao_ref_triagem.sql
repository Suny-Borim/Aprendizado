CREATE OR REPLACE PROCEDURE proc_atualizar_triagem_estudante_lista_area_atuacao(V_IDS_ESTUDANTES IN IDS_TYP DEFAULT NULL)
AS
    l_query  CLOB;
...
BEGIN
    ...
    l_query := l_query || '
            ...
            AND JSON_VALUE(respostas, ''$.tipo'') IN (''areaAtuacao'')) S
        ON (D.ID_ESTUDANTE = S.ID_ESTUDANTE)
        WHEN MATCHED THEN 
        UPDATE SET
            D.PAR_AREAS_ATUACAO         = S.AREAS_ATUACAO         -- areaAtuacao
            ,D.PAR_SEMESTRE_INICIO      = S.SEMESTRE_INICIO       -- areaAtuacao
            ,D.PAR_SEMESTRE_FIM         = S.SEMESTRE_FIM          -- areaAtuacao
