create or replace procedure PROC_BUSCA_CANDIDATOS(P_IDS_CANDIDATOS SERVICE_VAGAS_DEV.IDS_TYP default null,
...
begin
    ...
    if (P_IDS_CANDIDATOS is not null AND P_IDS_CANDIDATOS is not empty) then
        l_query := l_query || '
                   and TE.ID_ESTUDANTE member of (:P_IDS_CANDIDATOS)
                   ';
    else
        l_query := l_query || '
                   and (1 = 1  or   (:P_IDS_CANDIDATOS) is null )
                   ';
    end if;

    l_query := l_query || '
            AND (TE.PONTUACAO_OBTIDA <= :P_PONTUACAO_MAXIMA OR (:P_IDS_CANDIDATOS) is not empty)
