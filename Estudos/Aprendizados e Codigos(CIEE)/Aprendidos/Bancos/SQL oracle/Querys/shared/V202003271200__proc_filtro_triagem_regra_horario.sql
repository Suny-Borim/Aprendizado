create or replace PROCEDURE {{user}}."PROC_TRIAGEM_VAGAS_FILTRO"(v_id_estudante NUMBER,
                                             v_pcd NUMBER,
...
BEGIN
    ...
    from {{user}}.triagens_estudantes e
    cross join {{user}}.triagens_vagas v
    where e.id_estudante = :V_ID_ESTUDANTE

    -- Validar regra de estudante liberado da vaga
    ...
    IF (v_tipo_vaga = 'A' or v_tipo_vaga = 'AE') THEN
        ...
        (
            v.tipo_vaga = ''A''
            AND (E.CONTRATOS_EMPRESA IS EMPTY OR E.CONTRATOS_EMPRESA IS NULL OR NOT EXISTS (SELECT 1 from TABLE (E.CONTRATOS_EMPRESA) EC_EMP WHERE EC_EMP.ID_EMPRESA = V.ID_EMPRESA OR EC_EMP.TIPO_CONTRATO = ''A''))
            and
            (
                ...
                and (
                    ...
                    (v.situacao_escolaridade = 1 and e.status_escolaridade = 0) OR
                    (v.situacao_escolaridade = 2 and e.status_escolaridade = 1)
                )';
