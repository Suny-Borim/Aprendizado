...
BEGIN
    ...
    MERGE INTO {{user}}.TRIAGENS_ESTUDANTES D
    USING
        (SELECT
             EST.ID "ID_ESTUDANTE",
         ...
         FROM
             {{user}}.REP_ESTUDANTES EST
                 ...
                 LEFT JOIN {{user}}.REP_INFORMACOES_ADICIONAIS INFO ON INFO.ID = EST.ID
         ...
         WHERE
           --Filtra pelo estudante que deve ser atualizado
             (V_ID_ESTUDANTE IS NULL OR EST.ID = V_ID_ESTUDANTE)
           AND ENDER.PRINCIPAL = 1
           AND ESCOL.PRINCIPAL = 1
           AND (Q.ID IS NULL OR Q.NOME = 'Vulnerável')
           AND EST.DELETADO = '0'
