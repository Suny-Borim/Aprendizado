...
begin
    ...
    IF (P_CAPACITACAO.LATITUDE1 IS NOT NULL AND P_CAPACITACAO.LONGITUDE1 IS NOT NULL) THEN
        v_cap1_referencia := SDO_GEOMETRY(2001, 8307, SDO_POINT_TYPE(P_CAPACITACAO.LONGITUDE1, P_CAPACITACAO.LATITUDE1, NULL), NULL, NULL);
    END IF;

    IF (P_CAPACITACAO.LATITUDE2 IS NOT NULL AND P_CAPACITACAO.LONGITUDE2 IS NOT NULL) THEN
        v_cap2_referencia := SDO_GEOMETRY(2001, 8307, SDO_POINT_TYPE(P_CAPACITACAO.LONGITUDE2, P_CAPACITACAO.LATITUDE2, NULL), NULL, NULL);
    END IF;

    IF (P_CAPACITACAO.LATITUDE3 IS NOT NULL AND P_CAPACITACAO.LONGITUDE3 IS NOT NULL) THEN
    ...
    select te.id_estudante,
               ...
               (select id_cid_agrupado from table(pcds)) id_cid_agrupado,
    ...
    dbms_output.put_line(v_max_pontuacao);
