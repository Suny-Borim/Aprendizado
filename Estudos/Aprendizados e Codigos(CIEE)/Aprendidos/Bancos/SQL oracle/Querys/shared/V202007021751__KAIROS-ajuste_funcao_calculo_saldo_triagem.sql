...
BEGIN
    ...
    SELECT COUNT(1) INTO Contratados FROM SERVICE_VAGAS_DEV.VINCULOS_VAGA v WHERE v.codigo_vaga = p_codigo_da_vaga AND v.deletado = 0 AND v.SITUACAO_VINCULO IN (2,4,5);

    SELECT COUNT(1) INTO Encaminhados FROM SERVICE_VAGAS_DEV.VINCULOS_VAGA v WHERE v.codigo_vaga = p_codigo_da_vaga AND v.deletado = 0 AND v.SITUACAO_VINCULO = 1;
    ...
    WHERE t1.endereco_principal = 1 and t0.CODIGO_DA_VAGA = p_codigo_da_vaga
      AND ROWNUM < = 1;
