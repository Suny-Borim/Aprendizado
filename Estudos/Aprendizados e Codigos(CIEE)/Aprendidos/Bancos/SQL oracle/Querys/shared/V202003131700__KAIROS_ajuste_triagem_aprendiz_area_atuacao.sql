CREATE OR REPLACE PROCEDURE {{user}}.proc_atualizar_triagem_vaga_aprendiz
    (
...
BEGIN
    ...
    MERGE INTO {{user}}."TRIAGENS_VAGAS" D
        ...
        FROM
        {{user}}.PCD_APRENDIZ PCD
        WHERE
        PCD.ID_VAGA_APRENDIZ = V.ID
        AND PCD.deletado = 0
        ) as pcds,
        ...
        (
        SELECT
        CAST(COLLECT(APARELHO.ID_APARELHO) AS {{user}}.IDS_TYP)
        from
        {{user}}.APARELHOS_PCD_APRENDIZ APARELHO
        INNER JOIN {{user}}.PCD_APRENDIZ PA on APARELHO.ID_PCD = PA.ID
        WHERE
        PA.ID_VAGA_APRENDIZ = V.ID
        AND PA.deletado = 0
        ...
        FROM
        {{user}}.VINCULO_QUALI_VAGA_APRENDIZ VQVE
        WHERE
        V.ID = VQVE.ID_VAGA_APRENDIZ
        ) as QUALIFICACOES,
        ...
        {{user}}.GEOHASH_ENCODE(SDO_GEOM.SDO_CENTROID(END_EMP.GEO_POINT, 0.05).sdo_point.y,SDO_GEOM.SDO_CENTROID(END_EMP.GEO_POINT, 0.05).sdo_point.x, 4)
        ...
        {{user}}."GEOHASH_NEIGHBORS"(
        {{user}}.GEOHASH_ENCODE(T.LATITUDE, T.LONGITUDE,4)
        ...
        INNER JOIN {{user}}.REP_PARAMETROS_UNIDADES_CIEE PUC ON PPE.CODIGO_CIEE = PUC.ID_CIEE
        ...
        COUNT(1)
        FROM
        {{user}}.VINCULOS_VAGA vinculos
        WHERE
        vinculos.codigo_vaga = V.codigo_da_vaga
        ...
        CASE WHEN EXISTS(
        SELECT 1 FROM
        {{user}}.OCORRENCIAS_APRENDIZ OCORRENCIA
        WHERE
        OCORRENCIA.ID_VAGA_APRENDIZ = V.ID
        AND OCORRENCIA.deletado = 0
        ) THEN 1 ELSE 0 END as POSSUI_OCORRENCIAS,

        case when V.DIVULGAR_NOME_EMPRESA = 1 then
        CASE WHEN (INF_EMP.TIPO_EMPRESA IS NOT NULL AND INF_EMP.TIPO_EMPRESA = 'EMPRESA JURIDICA') OR INF_EMP.NOME IS NULL THEN INF_EMP.RAZAO_SOCIAL ELSE INF_EMP.NOME END
        ...
        INNER JOIN {{user}}.REP_INFO_CONTRATO_EMPRESAS INF_EMP   ON LOC.ID_CONTRATO = INF_EMP.ID_CONTRATO
        INNER JOIN {{user}}.TURMAS T                             ON T.ID_VAGA_APRENDIZ = V.ID
        INNER JOIN {{user}}.CURSOS_CAPACITACAO CC on V.ID_CURSO_CAPACITACAO = CC.ID
        WHERE
        (V_ID_VAGA_APRENDIZ IS NULL OR V.ID = V_ID_VAGA_APRENDIZ)
        ...
        AND LOC_END.ENDERECO_PRINCIPAL = 1
    ...
    WHEN  MATCHED THEN
    UPDATE SET
        D.ID_UNIDADE_CIEE = S.ID_UNIDADE_CIEE
