...
BEGIN
    ...
    MERGE INTO classificacoes_estudantes_analitico cea
       USING (SELECT E.ID, func_interpretar_expressao(REPLACE('$>=3', '$', COUNT(1)))
                ...
                WHERE P.id_estudante IN (SELECT COLUMN_VALUE FROM TABLE(p_lista_estudante))
                    ...
                    AND P.status = 1
                    AND ( datediff('DD', P.data_conclusao_prova, sysdate) >= 0 )
                    AND ( datediff('DD', P.data_conclusao_prova, sysdate) <= p_periodo )
                    AND P.deletado = 0
                    AND E.deletado = 0
