...
BEGIN
    ...
    MERGE INTO classificacoes_estudantes_analitico cea
       USING (SELECT e.ID, func_interpretar_expressao(REPLACE(p_faixa, '$', COUNT(1)))
                ...
                INNER JOIN rep_redacoes_student rd ON (rd.id_estudante = E.ID )
                WHERE rd.id_estudante IN (SELECT COLUMN_VALUE FROM TABLE(p_lista_estudante))
                    ...
                    AND ((CAST(rd.data_termino AS DATE) - CAST(sysdate AS DATE)) >= 0 )
                    AND ((CAST(rd.data_termino AS DATE) - CAST(sysdate AS DATE)) <= 365 )
                    AND E.deletado = 0
                    AND rd.deletado = 0
                GROUP BY e.ID) ceas
